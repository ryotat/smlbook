...
% Fix the xlabel
hxlabel=get(h,'xlabel');
if ~isempty(get(hxlabel,'string'))
  set(hx,'unit','points');
  tkext=median(cell2mat(get(hx,'Extent')));
  set(hx,'unit','data');
  set(hxlabel,'unit','points');
  xlpos=get(hxlabel,'Position');
  set(hxlabel, 'Position',[xlpos(1), xlpos(2)-tkext(4)]);
end
